...
import { LatLngPOI } from "../schemas/toursQueries.schema";
...
const handleImagePlaceholder = async (tourId, isProd) => {
  try {
    ...
    const rangeSlug =
      result.rows && result.rows.length > 0 ? result.rows[0].range_slug : null;
  ...
  if (ids) {
    ...
    try {
      if (isProd) {
        ...
        addParam.executablePath = path.resolve(
          __dirname,
          "../../node_modules/puppeteer/.local-chromium/linux-1022525/chrome-linux/chrome",
        );
      } else {
        dir_go_up = "../../../";
        url =
 ...
 */
export async function hashedUrlsFromPoi(latLngPOI: LatLngPOI) {
  try {
    const sql = `
    ...
    console.error(
      `Error obtaining tours within radius ${latLngPOI.radius} from lat=${latLngPOI.lat} and lon=${latLngPOI.lng}: `,
      e,
    );
    return null;
  }
